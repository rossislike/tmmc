...
resource "aws_ec2_transit_gateway_peering_attachment" "tokyo_to_california_tgw_peering_att" {
  ...
  tags = {
    Name = "tokyo_to_california"
  }
}

resource "aws_ec2_transit_gateway_route" "tokyo_to_california_tgw_rte" {
  provider                       = aws.tokyo
...
resource "aws_ec2_transit_gateway_route_table_association" "tokyo_to_california_tgw_rtb_assoc" {
  provider                       = aws.tokyo
  transit_gateway_attachment_id  = aws_ec2_transit_gateway_peering_attachment.tokyo_to_california_tgw_peering_att.id
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.tokyo_tgw_rtb.id
